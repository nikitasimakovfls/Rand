...
test.describe('Admin creates a Patient, full workflow', () => {
  ...
  test('Admin creates a Patient, Patient activates account, then Admin deletes Patient', async ({ browser }) => {
    ...
    // --- DATE INPUT BLOCK (Yesterday) ---
    const nowInClinic = DateTime.now().setZone('Europe/Belgrade');
    const yesterday = nowInClinic.minus({ days: 1 });

    await adminPageObj.page.locator('.react-datepicker__input-container input').click();
    ...
    await adminPageObj.page.locator(`.react-datepicker__day--0${yesterdayDay}:not(.react-datepicker__day--outside-month)`).first().click();

    await adminPageObj.page.locator('.react-datepicker__time-list-item', { hasText: '10:00 AM' }).click();
